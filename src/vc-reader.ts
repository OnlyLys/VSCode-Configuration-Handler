...
import { Values, ValuesWithEffectiveScope, EffectiveScope } from './values';
...
export class VCReader<T> {
     ...
     *
     * The effective value can possibly be `undefined` if all values (including the `defaultValue`)
     * fail validation. In order to get a guaranteed effective value, `guaranteedEffectiveValue` 
     * getter can be called. It will throw a `NoGuaranteedEffectiveValueError` if `effectiveValue` 
     * is `undefined`. 
     * 
    ...
    public read(scope?: ConfigurationScope): Values<T> {
        // The `effectiveScope` property will be dropped here.
        return this._read(scope);
    }

    /** **For internal use only.** */
    public _read(scope: ConfigurationScope | undefined): ValuesWithEffectiveScope<T> {
        ...
        return {
            ...
            effectiveValue,
            effectiveScope,
            get guaranteedEffectiveValue(): T {
                if (this.effectiveValue !== undefined) {
                    return this.effectiveValue;
                } else {
                    throw new NoGuaranteedEffectiveValueError(_this.args.name);
                }
            }
        };
    }
